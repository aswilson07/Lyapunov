...
{\bf Assumptions:} $f, h$ are convex and differentiable.\\
Choose $A_0 = 1$, $x_{-1}=x_0 = z_0$ and $\tau_{k+1} = \frac{A_{k+1} - A_k}{A_{k+1}}$. Define recursively,
\begin{subequations}
...
%\end{align*}
%Integrating shows~\eqref{Eq:LyapGrad} is a Lyapunov function for the gradient flow $\dot X_t = -\nabla f(X_t)$ when $f$ is $\mu$-strongly convex.
%
...
\end{equation}
however first, we analyze the dynamics to understand how one could obtain the Lyapunov function \eqref{Eq:Lyap3} for the dual-averaged algorithm.
 
 \paragraph{The Dynamic} We analyze the momentum dynamics~\eqref{Eq:ELBreg} with an additional increasing weighting term $\gamma_0\geq 0$, $\forall t\in \R$ (where $\dot \gamma_t \geq 0$):
